...
boat_image = cv2.imread('boat.png', cv2.IMREAD_UNCHANGED)

# cv2.imshow('Farm', farm_image)
# cv2.waitKey()
# cv2.destroyAllWindows()

#removing 4th channel from boat png
#hey = wheat_image.shape[2]
boat_image = cv2.cvtColor(boat_image, cv2.COLOR_BGRA2BGR)
# hello = boat_image.shape[2]
# print(hello)



# cv2.imshow('Chicken', chicken_image)
# cv2.waitKey()
# cv2.destroyAllWindows()

#result = cv2.matchTemplate(farm_image, wheat_image, cv2.TM_CCOEFF_NORMED)
result = cv2.matchTemplate(farm_image, boat_image, cv2.TM_CCOEFF_NORMED)
...
w = boat_image.shape[1]
h = boat_image.shape[0]
